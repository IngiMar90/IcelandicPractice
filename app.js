...
const STORAGE_KEY = "islenska_aefing_pwa_v1";
...
function loadDb() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY)) || { sessions: [], high_scores: {}, goals: {} };
  } catch {
    return { sessions: [], high_scores: {}, goals: {} };
  }
}
function saveDb() {
...
function renderMain() {
  ...
  const studentNames = [...new Set(db.sessions.map(s => s.student))].sort();
    ...
    <div class="screen">
      ...
      <div class="grid main-grid">
        <section class="card">
          <h2>Stillingar</h2>
          <div class="field">
            <label for="student">Nafn nemanda</label>
            <input id="student" type="text" placeholder="Sláðu inn nafn">
          </div>
          <div class="field">
...
function renderTeacherArea() {
  const app = document.getElementById("app");
  const students = [...new Set(db.sessions.map(s => s.student))].sort();
